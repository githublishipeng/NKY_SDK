package com.nky.sdk_flutter.nkysdk.protocal.version7;
...
public class Protocol0X26 extends Protocol {
     ...
     */
    public static Protocol0X26 newInstance(long fileTotalCount, long currentPackageNo, byte[] _fileData) {
        Protocol0X26 protocol = new Protocol0X26();
        protocol._fileTotalCount = ByteUtils.longToBytes4(fileTotalCount);
        protocol._currentPackageNo = ByteUtils.longToBytes4(currentPackageNo);
    ...
    private final static int FILE_TOTAL_COUNT = 4;

    /**
     * 数据串内容：当前数据包编号，从1开始，编号为0是20字节校验内容
     */
    private byte[] _currentPackageNo;
    private final static int CURRENT_PACKAGE_NO = 4;
    ...
    public static Result parse(byte[] response) {
        ...
        int currentPackageNo = ByteUtils.convert4BytesToUnsignedInt(new byte[]{result[4],result[5],result[6], result[7]});
        int status = ByteUtils.byteToUnsignedInt(result[8]);
